    ...
    txdelegate(new TxViewDelegate(platformStyle, this))
{
    ...
    // Wrap left column (verticalLayout_2) in QScrollArea for scrollable asset list
    {
        // Create scroll area
        ...
        m_assetLayout = new QVBoxLayout(scrollContent);
        m_assetLayout->setContentsMargins(0, 0, 0, 0);

        // Move GRD balance frame (ui->frame) into scroll content
        ui->verticalLayout_2->removeWidget(ui->frame);
        m_assetLayout->addWidget(ui->frame);

        // Add stretch at bottom
        m_assetLayout->addStretch();

        scrollContent->setLayout(m_assetLayout);
        m_scrollArea->setWidget(scrollContent);

        // Remove old spacer from verticalLayout_2
        QLayoutItem *spacer = ui->verticalLayout_2->takeAt(0);
        if (spacer) delete spacer;

        // Add scroll area to the left column
        ui->verticalLayout_2->addWidget(m_scrollArea);
    }
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // Token holdings — refresh now, then every 30 seconds
    refreshTokenHoldings();
    if (!m_tokenTimer) {
        m_tokenTimer = new QTimer(this);
        m_tokenTimer->setInterval(30000);
        connect(m_tokenTimer, &QTimer::timeout, this, &OverviewPage::refreshTokenHoldings);
        m_tokenTimer->start();
    }
}
...
// ── Kepemilikan Token — dynamic Balances frame per asset ─────────────────────
void OverviewPage::refreshTokenHoldings()
{
    if (!clientModel) return;

    // Remove old asset frames
    for (auto* f : m_assetFrames) {
        m_assetLayout->removeWidget(f);
        delete f;
    }
    m_assetFrames.clear();
    ...
    try {
        ...
        // Aggregate balance per symbol (sum across addresses)
        struct AssetSummary {
            QString symbol;
            QString type;
            double  balance = 0;
        };
        std::map<std::string, AssetSummary> merged;

        for (size_t i = 0; i < result.size(); ++i) {
            const UniValue &a = result[i];
            std::string sym = a["symbol"].get_str();
            double bal = std::atof(a["balance"].get_str().c_str());
            auto &s = merged[sym];
            s.symbol  = QString::fromStdString(sym);
            s.type    = a.exists("type") ? QString::fromStdString(a["type"].get_str()) : "token";
            s.balance += bal;
        }

        // Insert position: after the GRD balance frame (index 1 in m_assetLayout)
        int insertIdx = 1;

        for (const auto& [key, asset] : merged) {
            ...
            // Header: "Balances"
            QLabel *hdr = new QLabel(frame);
            QFont boldFont;
            boldFont.setBold(true);
            hdr->setFont(boldFont);
            hdr->setText(QStringLiteral("Balances"));

            QHBoxLayout *hdrLayout = new QHBoxLayout();
            hdrLayout->addWidget(hdr);
            hdrLayout->addStretch();
            vbox->addLayout(hdrLayout);

            // Grid: Available / Pending / Total
            QGridLayout *grid = new QGridLayout();
            grid->setSpacing(12);

            QString balStr = QString::number(asset.balance, 'f', 8);
            QString unit = asset.symbol;

            QLabel *lblAvailText = new QLabel(QStringLiteral("Available:"), frame);
            QLabel *lblAvailVal  = new QLabel(balStr + " " + unit, frame);
            lblAvailVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
            lblAvailVal->setCursor(Qt::IBeamCursor);
            lblAvailVal->setTextInteractionFlags(Qt::TextSelectableByMouse);
            grid->addWidget(lblAvailText, 0, 0);
            grid->addWidget(lblAvailVal,  0, 1);

            QLabel *lblPendText = new QLabel(QStringLiteral("Pending:"), frame);
            QLabel *lblPendVal  = new QLabel(QStringLiteral("0.00000000 ") + unit, frame);
            lblPendVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
            grid->addWidget(lblPendText, 1, 0);
            grid->addWidget(lblPendVal,  1, 1);
            ...
            QLabel *lblTotalText = new QLabel(QStringLiteral("Total:"), frame);
            QLabel *lblTotalVal  = new QLabel(balStr + " " + unit, frame);
            lblTotalVal->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
            lblTotalVal->setCursor(Qt::IBeamCursor);
            lblTotalVal->setTextInteractionFlags(Qt::TextSelectableByMouse);
            grid->addWidget(lblTotalText, 3, 0);
            grid->addWidget(lblTotalVal,  3, 1);

            vbox->addLayout(grid);
            frame->setLayout(vbox);

            m_assetLayout->insertWidget(insertIdx, frame);
            m_assetFrames.push_back(frame);
            insertIdx++;
        }

    } catch (...) {
        // RPC tidak tersedia — tidak tampilkan apa-apa
    }
}
