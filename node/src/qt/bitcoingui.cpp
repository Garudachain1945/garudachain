...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a GarudaChain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and garuda: URIs)"));
    ...
    // Mode-specific tabs — read from gArgs directly because SetWalletMode
    // runs in AppInitParameterInteraction which is called AFTER the GUI is created.
    CBDC::SetWalletMode(gArgs.GetArg("-walletmode", "normal"));
    CBDC::WalletMode wmode = CBDC::GetWalletMode();
    if (wmode == CBDC::WalletMode::CBDC) {
        mintBurnAction = new QAction(platformStyle->SingleColorIcon(":/icons/send"), tr("&Stablecoin"), this);
        mintBurnAction->setStatusTip(tr("Terbitkan, mint, dan burn stablecoin"));
        mintBurnAction->setToolTip(mintBurnAction->statusTip());
        mintBurnAction->setCheckable(true);
        mintBurnAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
        tabGroup->addAction(mintBurnAction);
    }
    if (wmode == CBDC::WalletMode::CREATOR) {
        createTokenAction = new QAction(platformStyle->SingleColorIcon(":/icons/add"), tr("&Saham"), this);
        createTokenAction->setStatusTip(tr("Buat token saham, dividen, dan e-IPO"));
        createTokenAction->setToolTip(createTokenAction->statusTip());
        createTokenAction->setCheckable(true);
        createTokenAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
        tabGroup->addAction(createTokenAction);
    }
    if (wmode == CBDC::WalletMode::PUBLIC) {
        investAction = new QAction(platformStyle->SingleColorIcon(":/icons/overview"), tr("&Invest"), this);
        investAction->setStatusTip(tr("e-IPO, portofolio saham, dan swap di GarudaDEX"));
        investAction->setToolTip(investAction->statusTip());
        investAction->setCheckable(true);
        investAction->setShortcut(QKeySequence(QStringLiteral("Alt+5")));
    ...
    if (mintBurnAction) {
        connect(mintBurnAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
        connect(mintBurnAction, &QAction::triggered, this, &BitcoinGUI::gotoMintBurnPage);
    }
    if (createTokenAction) {
        connect(createTokenAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
        connect(createTokenAction, &QAction::triggered, this, &BitcoinGUI::gotoCreateTokenPage);
    }
    if (investAction) {
        connect(investAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
        connect(investAction, &QAction::triggered, this, &BitcoinGUI::gotoInvestPage);
    }
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your GarudaChain addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified GarudaChain addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed GarudaChain Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed GarudaChain Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a garuda: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible GarudaChain command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if (mintBurnAction)    toolbar->addAction(mintBurnAction);
        if (createTokenAction) toolbar->addAction(createTokenAction);
        if (investAction)      toolbar->addAction(investAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    if (mintBurnAction)    mintBurnAction->setEnabled(enabled);
    if (createTokenAction) createTokenAction->setEnabled(enabled);
    if (investAction)      investAction->setEnabled(enabled);
}
...
void BitcoinGUI::gotoMintBurnPage()
{
    if (mintBurnAction) mintBurnAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMintBurnPage();
}

void BitcoinGUI::gotoCreateTokenPage()
{
    if (createTokenAction) createTokenAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateTokenPage();
}

void BitcoinGUI::gotoInvestPage()
{
    if (investAction) investAction->setChecked(true);
    if (walletFrame) walletFrame->gotoInvestPage();
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to GarudaChain network.", "", count);
    } else {
        //: A substring of the tooltip.
...
{
    // Show mode-specific name only — no wallet name, no network tag
    QString window_title;
    CBDC::WalletMode wmode = CBDC::GetWalletMode();
    if (wmode == CBDC::WalletMode::CBDC) {
        window_title = tr("CBDC Core");
    } else if (wmode == CBDC::WalletMode::CREATOR) {
        window_title = tr("Creator Core");
    } else if (wmode == CBDC::WalletMode::PUBLIC) {
        window_title = tr("Public Core");
    } else {
