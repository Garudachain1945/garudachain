      ...
      platformStyle(_platformStyle)
{
    ...
    // Create mode-specific pages based on walletmode
    CBDC::WalletMode wmode = CBDC::GetWalletMode();
    if (wmode == CBDC::WalletMode::CBDC) {
        mintBurnPage = new MintBurnPage(platformStyle);
        mintBurnPage->setWalletModel(walletModel);
    }
    if (wmode == CBDC::WalletMode::CREATOR) {
        createTokenPage = new CreateTokenPage(platformStyle);
        createTokenPage->setWalletModel(walletModel);
    }
    if (wmode == CBDC::WalletMode::PUBLIC) {
        investPage = new InvestPage(platformStyle);
        investPage->setWalletModel(walletModel);
    }
    ...
    if (mintBurnPage)    addWidget(mintBurnPage);
    if (createTokenPage) addWidget(createTokenPage);
    if (investPage)      addWidget(investPage);
...
void WalletView::setClientModel(ClientModel *_clientModel)
{
    ...
    if (mintBurnPage)    mintBurnPage->setClientModel(_clientModel);
    if (createTokenPage) createTokenPage->setClientModel(_clientModel);
    if (investPage)      investPage->setClientModel(_clientModel);
...
void WalletView::gotoMintBurnPage()
{
    if (mintBurnPage) setCurrentWidget(mintBurnPage);
}

void WalletView::gotoCreateTokenPage()
{
    if (createTokenPage) setCurrentWidget(createTokenPage);
}

void WalletView::gotoInvestPage()
{
    if (investPage) setCurrentWidget(investPage);
}
