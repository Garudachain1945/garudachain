...
const QString BITCOIN_IPC_PREFIX("garuda:");
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click garuda: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start garuda: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("garuda://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'garuda://' is not a valid URI. Use 'garuda:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // garuda: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid GarudaChain address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
