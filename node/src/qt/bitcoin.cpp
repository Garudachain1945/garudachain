...
void BitcoinApplication::setupPlatformStyle()
{
    ...
    // ── GarudaChain: ganti warna biru → merah bendera Indonesia ──────────
    // Merah Bendera Indonesia: #CE1126
    QPalette pal = QApplication::palette();
    pal.setColor(QPalette::Highlight,        QColor(0xCE, 0x11, 0x26));
    pal.setColor(QPalette::HighlightedText,  Qt::white);
    pal.setColor(QPalette::Link,             QColor(0xCE, 0x11, 0x26));
    QApplication::setPalette(pal);

    // Global stylesheet: progress bar, selection, focus, scrollbar
    QApplication::setStyleSheet(
        // Progress bar bawah (sync)
        "QProgressBar {"
        ...
        "  border-radius: 4px;"
        "}"
        // Selection highlight di list/table
        "QListView::item:selected, QTableView::item:selected {"
        "  background-color: #CE1126;"
        "  color: white;"
        "}"
        // Link color
        "QLabel a { color: #CE1126; }"
