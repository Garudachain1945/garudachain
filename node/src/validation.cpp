...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // do all inputs exist?
    // CBDC_MINT txs have a synthetic marker prevout — skip UTXO lookup for them.
    // Schnorr signature verification is done at ConnectBlock time.
    if (!CBDC::IsCBDCMintTx(tx)) {
        for (const CTxIn& txin : tx.vin) {
    ...
    // CBDC_MINT: skip BIP68 and input checks — marker prevout has no real UTXO
    ...
    if (!CBDC::IsCBDCMintTx(tx) && !Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_chain.Height() + 1, ws.m_base_fees)) {
        return false; // state filled in by CheckTxInputs
    }

    if (!CBDC::IsCBDCMintTx(tx) && m_pool.m_opts.require_standard && !AreInputsStandard(tx, m_view)) {
        return state.Invalid(TxValidationResult::TX_INPUTS_NOT_STANDARD, "bad-txns-nonstandard-inputs");
    }

    // Check for non-standard witnesses.
    if (!CBDC::IsCBDCMintTx(tx) && tx.HasWitness() && m_pool.m_opts.require_standard && !IsWitnessStandard(tx, m_view)) {
        return state.Invalid(TxValidationResult::TX_WITNESS_MUTATED, "bad-witness-nonstandard");
    }

    int64_t nSigOpsCost = CBDC::IsCBDCMintTx(tx) ? 0 : GetTransactionSigOpCost(tx, m_view, STANDARD_SCRIPT_VERIFY_FLAGS);
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // CBDC_MINT: skip script checks — no real inputs, authority sig verified at ConnectBlock
    if (!CBDC::IsCBDCMintTx(tx) && !CheckInputScripts(tx, state, m_view, scriptVerifyFlags, true, false, ws.m_precomputed_txdata, GetValidationCache())) {
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // CBDC_MINT: no real inputs to check scripts on
    if (CBDC::IsCBDCMintTx(tx)) return true;
...
{
    // GarudaChain: 0.01 GRD per block, unlimited supply, no halving
    if (nHeight == 0) return 10 * COIN; // genesis pre-mine

    // Hardcoded historical sovereign mints (height -> amount in whole GRD).
    // These are baked into consensus so every node validates the chain the same way.
    static const std::map<int, long long> HISTORICAL_MINTS = {
        {161, 1000000000LL}, // 1 billion GRD
        {257, 1000000000LL}, // 1 billion GRD
    };
    if (auto it = HISTORICAL_MINTS.find(nHeight); it != HISTORICAL_MINTS.end()) {
        return CAmount(it->second) * COIN;
    }

    // Pending sovereign mint file: /tmp/garuda-mint may contain one or more
    // "<height> <amount_grd>" lines. Matches activate the mint at that height.
    {
        std::ifstream f("/tmp/garuda-mint");
        if (f.is_open()) {
            long long th, amt;
            while (f >> th >> amt) {
                if (th == (long long)nHeight && amt > 0) {
                    LogPrintf("GarudaMint: pending mint ACTIVATED at height %d, subsidy %lld GRD\n", nHeight, amt);
                    return CAmount(amt) * COIN;
                }
            }
        }
    }
    return COIN / 100; // 0.01 GRD
}
...
{
    // mark inputs spent
    // CBDC_MINT txs have a marker prevout that doesn't exist in the UTXO set — treat like coinbase
    if (!tx.IsCoinBase() && !CBDC::IsCBDCMintTx(tx)) {
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // violating blocks.
    // GarudaChain: SCRIPT_VERIFY_QUANTUM is always active from genesis —
    // P2PQH (witness v2, ML-DSA-87) is a native address type, not a soft fork.
    // Without this flag, ConnectBlock() skips quantum signature verification
    // and a miner could include P2PQH inputs with forged signatures.
    uint32_t flags{SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_TAPROOT | SCRIPT_VERIFY_QUANTUM};
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // CBDC rate limiting: track per-block mint totals
    int32_t cbdc_mint_count = 0;
    int64_t cbdc_mint_total = 0;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            // CBDC_MINT: skip normal UTXO input validation, verify authority Schnorr sig instead
            if (CBDC::IsCBDCMintTx(tx)) {
                if (!CBDC::CheckCBDCMintInBlock(tx, state)) {
                    LogError("%s: CBDC_MINT tx %s invalid\n", __func__, tx.GetHash().ToString());
                    return false;
                }
                // Per-block rate limiting
                cbdc_mint_count++;
                if (cbdc_mint_count > CBDC::MAX_MINT_TXS_PER_BLOCK) {
                    LogError("%s: too many CBDC_MINT txs in block (%d > %d)\n",
                             __func__, cbdc_mint_count, CBDC::MAX_MINT_TXS_PER_BLOCK);
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                        "cbdc-mint-too-many-per-block");
                }
                for (const auto& out : tx.vout) cbdc_mint_total += out.nValue;
                if (cbdc_mint_total > CBDC::MAX_MINT_PER_BLOCK) {
                    LogError("%s: CBDC_MINT total in block exceeds limit (%ld > %ld)\n",
                             __func__, cbdc_mint_total, CBDC::MAX_MINT_PER_BLOCK);
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                        "cbdc-mint-exceeds-block-limit");
                }
                // CBDC_MINT does not pay fees and has no real inputs — skip to output processing
            } else {
                CAmount txfee = 0;
        ...
        // CBDC_MINT: no real inputs, skip sigop counting that accesses coins
        if (!CBDC::IsCBDCMintTx(tx))
