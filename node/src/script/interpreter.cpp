...
static bool EvalChecksig(const valtype& sig, const valtype& pubkey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, ScriptExecutionData& execdata, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& success)
{
    ...
    case SigVersion::QUANTUM:
        // Quantum verification is handled directly in VerifyWitnessProgram, not via script execution.
        break;
    }
    assert(false);
}
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckQuantumSignature(Span<const unsigned char> sig, Span<const unsigned char> pubkey, const CScript& scriptCode, ScriptError* serror) const
{
    // ML-DSA-87 signature verification for witness v2 (P2PQH)
    if (pubkey.size() != PQC::ML_DSA_87_PUBKEY_SIZE) {
        return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    }
    if (sig.size() != PQC::ML_DSA_87_SIG_SIZE) {
        return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    }

    // Compute BIP143-style sighash using SIGHASH_ALL
    // The mobile/extension wallets use BIP143 format with witness v0 sighash computation
    if (amount < 0) return HandleMissingData(m_mdb);
    if (!txTo) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    uint256 sighash = SignatureHash(scriptCode, *txTo, nIn, SIGHASH_ALL, amount, SigVersion::WITNESS_V0, this->txdata);

    // Verify ML-DSA-87 signature over the 32-byte sighash
    if (!PQC::MLDSA87Verify(sighash.begin(), 32, sig.data(), sig.size(), pubkey.data())) {
        return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
    }
    return true;
}
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // GarudaChain P2PQH: Post-Quantum Hash (ML-DSA-87)
        // scriptPubKey: OP_2 <32-byte SHA256(pubkey)>
        // witness stack: [signature, pubkey]
        if (!(flags & SCRIPT_VERIFY_QUANTUM)) return set_success(serror);
        if (stack.size() != 2) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        const valtype& sig_data = stack[0];
        const valtype& pubkey_data = stack[1];

        // Verify pubkey size is ML-DSA-87 (2592 bytes)
        if (pubkey_data.size() != PQC::ML_DSA_87_PUBKEY_SIZE) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }

        // Verify SHA256(pubkey) matches the 32-byte witness program
        uint256 pubkey_hash;
        CSHA256().Write(pubkey_data.data(), pubkey_data.size()).Finalize(pubkey_hash.begin());
        if (memcmp(pubkey_hash.begin(), program.data(), 32) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }

        // Build scriptCode for BIP143 sighash: OP_2 <push 32> <program>
        CScript quantum_script;
        quantum_script << OP_2;
        quantum_script << std::vector<unsigned char>(program.begin(), program.end());

        // Verify ML-DSA-87 signature
        if (!checker.CheckQuantumSignature(sig_data, pubkey_data, quantum_script, serror)) {
            return false; // serror already set
        }
        return set_success(serror);
