        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    UniValue result = generateBlocks(chainman, miner, coinbase_script, num_blocks, max_tries);

    // Auto-forward APBN fee ke market maker setelah mining
    try {
        std::any ctx = request.context;
        int64_t forwarded = APBN::AutoForwardToMarketMakers(request.URI, &ctx);
        if (forwarded > 0) {
            LogPrintf("APBN: auto-forwarded %d sat ke market maker\n", forwarded);
        }
    } catch (...) {
        // Silent fail — don't break mining if forward fails
    }
...
// ---------------------------------------------------------------------------
// apbnforward — manual trigger: forward APBN balance ke market maker
// ---------------------------------------------------------------------------
static RPCHelpMan apbnforward()
{
    return RPCHelpMan{
        "apbnforward",
        "Forward saldo APBN ke market maker secara manual.\n"
        "Otomatis dipanggil setiap kali block ditambang, tapi bisa juga dipanggil manual.\n"
        "30% fee transaksi masuk ke APBN, lalu APBN forward ke market maker.\n",
        {},
        RPCResult{RPCResult::Type::OBJ, "", "", {}},
        RPCExamples{HelpExampleCli("apbnforward", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            std::any ctx = request.context;
            int64_t forwarded = APBN::AutoForwardToMarketMakers(request.URI, &ctx);
            ...
            UniValue result(UniValue::VOBJ);
            result.pushKV("apbn_address", APBN::APBN_ADDRESS);
            result.pushKV("fee_percent", APBN::FEE_PERCENT);
            char buf[32];
            snprintf(buf, sizeof(buf), "%.8f", (double)forwarded / 100000000.0);
            result.pushKV("forwarded_grd", std::string(buf));
            result.pushKV("forwarded_sat", forwarded);
            result.pushKV("status", forwarded > 0 ? "OK" : "nothing_to_forward");
            return result;
        },
    };
}
...
// ---------------------------------------------------------------------------
// apbnstatus — lihat status APBN: saldo, fee %, active MMs
// ---------------------------------------------------------------------------
static RPCHelpMan apbnstatus()
{
    return RPCHelpMan{
        "apbnstatus",
        "Lihat status APBN (treasury).\n"
        "Menampilkan saldo APBN, % fee, dan market maker yang menerima dana.\n",
        {},
        RPCResult{RPCResult::Type::OBJ, "", "", {}},
        RPCExamples{HelpExampleCli("apbnstatus", "")},
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            // Get APBN balance via listunspent
            JSONRPCRequest list_req;
            list_req.context = request.context;
            list_req.strMethod = "listunspent";
            list_req.params = UniValue(UniValue::VARR);
            list_req.params.push_back(0);
            list_req.params.push_back(9999999);
            UniValue addr_filter(UniValue::VARR);
            addr_filter.push_back(APBN::APBN_ADDRESS);
            list_req.params.push_back(addr_filter);
            list_req.URI = request.URI;

            int64_t balance_sat = 0;
            int utxo_count = 0;
            try {
                UniValue utxos = tableRPC.execute(list_req);
                if (utxos.isArray()) {
                    utxo_count = utxos.size();
                    for (size_t i = 0; i < utxos.size(); ++i) {
                        balance_sat += (int64_t)(utxos[i]["amount"].get_real() * 100000000.0);
                    }
                }
            } catch (...) {}

            // Get active market makers
            DEX::DexDB& dex_db = DEX::GetDexDB();
            auto mms = dex_db.ListMarketMakers(200);
            int active = 0;
            UniValue mm_list(UniValue::VARR);
            for (const auto& mm : mms) {
                if (!mm.active) continue;
                ++active;
                UniValue m(UniValue::VOBJ);
                m.pushKV("asset_id", mm.asset_id.GetHex());
                m.pushKV("mm_address", mm.mm_address);
                mm_list.push_back(m);
            }
            ...
            result.pushKV("fee_percent", APBN::FEE_PERCENT);
            char buf[32];
            snprintf(buf, sizeof(buf), "%.8f", (double)balance_sat / 100000000.0);
            result.pushKV("balance_grd", std::string(buf));
            result.pushKV("balance_sat", balance_sat);
            result.pushKV("utxo_count", utxo_count);
            result.pushKV("min_forward_sat", APBN::MIN_FORWARD_AMOUNT);
            result.pushKV("active_market_makers", active);
            result.pushKV("market_makers", mm_list);
            result.pushKV("auto_forward", "enabled (setiap block baru)");
            return result;
        },
    };
}
